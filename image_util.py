...
class BaseDataProvider(object):
    ...
    def _load_data_and_label(self):
        data, labels, weights = self._next_data()
            
        train_data = data#self._process_data(data)
        #labels = self._process_labels(label)
        #weights = self._process_data(weights)
        ...
        wx, wy = weights.shape[0:2]

        return train_data.reshape(1, ny, nx, self.channels), labels.reshape(1, wx, wy, self.n_class), weights.reshape(1, wx, wy, 1)
    ...
    def __call__(self, n):
        ...
        Y = np.zeros((n, wx, wy, self.channels))
        W = np.zeros((n, wx, wy, 1))
...
class ImageDataProvider(BaseDataProvider):
    ...
    def __init__(self, search_path, a_min=None, a_max=None, data_suffix=".tif", mask_suffix='_mask.tif', weight_suffix=None, shuffle_data=True, n_class = 2):
    ...
    def _find_data_files(self, search_path):
        ...
        return [name for name in all_files if self.data_suffix in name and not (self.mask_suffix in name or self.weight_suffix in name)]
    
    def _get_number_of_files(self):
        return len(self.data_files)

    def _load_file(self, path, dtype=np.float32):
        return np.load(path)#np.array(Image.open(path), dtype)
    ...
    def _next_data(self):
        ...
        if self.weight_suffix is not None:
            weight_name = image_name.replace(self.data_suffix, self.weight_suffix)
            weights = self._load_file(weight_name, np.float32)
